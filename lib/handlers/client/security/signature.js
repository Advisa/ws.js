...
Signature.prototype.applyMe = function(doc, _security) {
  ...
  var sigNode = newDoc.importNode(sigDoc.documentElement, true);
	...
	var security = select(newDoc, "/*[local-name(.)='Envelope']/*[local-name(.)='Header']/*[local-name(.)='Security']")[0];
...
function WSSKeyInfo(signingToken) {
  ...
  this.getKeyInfo = function(key) {
    return "<wsse:SecurityTokenReference>" +
           "<wsse:Reference URI=\"#" + signingToken.getId() +"\" " +
